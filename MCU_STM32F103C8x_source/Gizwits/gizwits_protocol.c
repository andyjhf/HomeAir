...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueTVOC != cur->valueTVOC)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTVOC Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueHCHO = gizY2XFloat(HCHO_RATIO,  HCHO_ADDITION, dataPoints->valueHCHO); 
    ...
    devStatusPtr->valueTVOC = exchangeBytes(gizY2XFloat(TVOC_RATIO,  TVOC_ADDITION, dataPoints->valueTVOC));     
